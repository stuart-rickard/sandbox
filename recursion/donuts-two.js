...
var maxHappyGroups = function (batchSize, groups) {
  ...
  function lookForSets(currentSetSize, currentGroups, currentSum, batchSize) {
    ...
    if (currentSetSize == 1) {
      for (let i = 0; i < currentGroups.length; i++) {
        ...
        if (currentSum + currentGroups[i] == batchSize) {
  ...
  if (groups.length) {
    for (
      let multiplier = 1;
      multiplier * batchSize < groups.reduce((a, b) => a + b);
      multiplier++
    ) {
      ...
      for (let setSize = 1; setSize <= groups.length; setSize++) {
        ...
        groups.sort(
          (a, b) =>
            (b % (batchSize * multiplier)) - (a % (batchSize * multiplier))
        );
        for (let d = 0; d < groups.length; d++) {
          groups[d] = groups[d] % (batchSize * multiplier);
        }
        for (let j = 0; j < groups.length; j++) {
          let resultOfLook = lookForSets(
            setSize,
            groups,
            0,
